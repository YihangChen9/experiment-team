#!/usr/bin/env bash
# Fast query: cached remote machine / conda / configured assets / Hugging Face setup (POST /api/remote_setup).
# This reads whatever setup snapshot the remote API exposes. If expected fields are
# missing, continue with the available response or report the missing snapshot to the
# service operator.
#
# Usage:
#   ./fast_query_server_info.sh [--url URL | -u URL] [--key KEY | -k KEY] [--save | --no-save | -n]
#
...
OUT_DIR="${INFRA_QUERY_LOG_DIR:-$SCRIPT_DIR/infra_query_logs}"
...
curl -sS -X POST "$URL/api/remote_setup" -H "Content-Type: application/json" \
  -d "{\"session_key\":\"$KEY\"}" >"$tmp"

if [[ "$SAVE" == true ]]; then
  mkdir -p "$OUT_DIR"
  dest="$OUT_DIR/server_info_${TS}.json"
