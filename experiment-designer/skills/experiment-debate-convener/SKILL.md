...
### Step 9.2.5 — VERIFY the pin before writing it (MANDATORY)

A pin you have not verified is a hallucination with a SHA-shaped
string in it. A real run (083041abd013) pinned a commit that did not
exist and an adaptation-surface file that was not in the repo — the
Stage 6 implementer burned every retry on it. Before Step 9.3, run:

```bash
# 1. The repo exists and THIS is its real HEAD SHA (never type a SHA from memory):
git ls-remote https://github.com/<org>/<repo>.git HEAD

# 2. Every adaptation-surface file actually exists at that SHA:
git clone --depth 1 https://github.com/<org>/<repo>.git /tmp/pin_verify
cd /tmp/pin_verify && git ls-tree -r --name-only HEAD | grep -E "<file1>|<file2>|..."
```

Hard rules:
- The pin's `Commit:` field MUST be a SHA copied from the `ls-remote` /
  `git log` output above — never recalled, never invented, never copied
  from this runbook's examples.
- Every file in the Adaptation surface table MUST appear in the
  `ls-tree` output. A file you "remember" the repo having does not count.
- Paste both command outputs into the pin file under a `## Verification`
  section. A pin without a Verification section is incomplete — the
  Stage 5 critic and the Stage 6 implementer treat it as unverified.

### Step 9.3 — Produce `stage5_codebase_pin.md`

Write a short pin document the Stage 6 code-writer will consume verbatim.

> ⚠️ **Every name below is a PLACEHOLDER.** Do NOT copy this template's
> repo / file / SHA values into your pin — an earlier version of this
> example used a real-looking repo + file path, and a Stage 5 agent
> working on a matching topic copied them verbatim (including a
> nonexistent file), causing a full Stage 6 failure. Your pin's values
> must come from YOUR Step 9.1 search + Step 9.2.5 verification.
...
- Commit: <sha copied from your git ls-remote output>  (a specific SHA, NOT `main`)
- License: <MIT / Apache-2.0 / BSD — as found in the repo's LICENSE file>
- Test command: `<the repo's own documented test command>`

## Verification
<paste your Step 9.2.5 `git ls-remote` + `git ls-tree | grep` outputs here>
...
|------|--------|--------|---------------|
| `<path/that/appears/in/your/ls-tree/output.py>` | <change> | <reason> | <N> |
| `<another/verified/path.py>` | <change> | <reason> | <N> |

**Total adaptation surface: ~<N> LOC across <K> files.**
Files NOT to touch (use as-is): <the repo's battle-tested
extractor / scorer modules, named from the ls-tree output>.
